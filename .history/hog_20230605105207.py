...
def to_csv():
    ...
    for path in files:
        ...
        for image in all_images:
            ...
            # xác định đường dẫn đầu ra cho từng đặc trưng
            name = image.split("\\")
            color_mean_output_path = os.path.join(
                "Feature", 'color_mean_' + name[-1].split('.')[0] + '.npy')
            color_std_output_path = os.path.join(
                "Feature", 'color_out_' + name[-1].split('.')[0] + '.npy')
            hog_output_path = os.path.join(
                "Feature", 'hog_' + name[-1].split('.')[0] + '.npy')

            # lưu cấc đặc trưng vào file tương ứng
            np.save(color_mean_output_path, color_mean)
            np.save(color_std_output_path, color_std)
            np.save(hog_output_path, hog)

            color_std_output_path_csv = os.path.join(
                "Feature\\csv\\color_out", 'color_out_' + name[-1].split('.')[0] + '.csv')
            color_std_output_path_csv = open(color_std_output_path_csv, "w")
            color_std = [str(f[0]) for f in color_std]
            color_std_output_path_csv.write("%s\n" % (",".join(color_std)))
            color_std_output_path_csv.close()

            color_mean_output_path_csv = os.path.join(
                "Feature\\csv\\color_mean", 'color_mean_' + name[-1].split('.')[0] + '.csv')
            color_mean_output_path_csv = open(color_mean_output_path_csv, "w")
            color_mean = [str(f[0]) for f in color_mean]
            color_mean_output_path_csv.write("%s\n" % (",".join(color_mean)))
            color_mean_output_path_csv.close()

            hog_output_path_csv = os.path.join(
                "Feature\\csv\\hog", 'hog_' + name[-1].split('.')[0] + '.csv')
            hog_output_path_csv = open(hog_output_path_csv, "w")
            hog = [str(f) for f in hog]
            hog_output_path_csv.write("%s\n" % (",".join(hog)))
            hog_output_path_csv.close()

            # thêm vào danh sách tên hình ảnh
            data.append([image, color_mean_output_path,
                        color_std_output_path, hog_output_path])
